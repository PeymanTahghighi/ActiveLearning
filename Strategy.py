...
def get_cluster_centers(X, K, data):
...
def get_grad_embeddings(X, model):
    model.eval();
    embed_dim = model.get_embedd_dim();
    embeddings = np.zeros((len(X), embed_dim * 3));
    loader = DataLoader(NetworkDataset(X, None, Config.train_transforms), batch_size= BATCH_SIZE, shuffle= True, num_workers= NUM_WORKERS);
    with torch.no_grad():
        for x , idxs in loader:
            x  = x.to(DEVICE);
            cout, out = model(x);
            out = out.data.cpu().numpy()
            batchProbs = torch.softmax(cout, dim=1).data.cpu().numpy()
            maxInds = np.argmax(batchProbs,1)
            cls = np.where(maxInds==0, -1, cls);
            for j in range(len(x)):
                for c in range(nLab):
                    if c == maxInds[j]:
                        embeddings[idxs[j]][embed_dim * c : embed_dim * (c+1)] = np.sum(out[j] * (1 - batchProbs[j][c]), axis=(1,2), keepdims=False);
                    else:
                        embeddings[idxs[j]][embed_dim * c : embed_dim * (c+1)] = np.sum(out[j] * (-1 * batchProbs[j][c]), axis=(1,2), keepdims=False)
